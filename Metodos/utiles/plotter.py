...
def plot_fx_puntos(funcion, xinicial, xfinal, puntosx, puntosy, alias_punto):
    ...
    fig.add_trace(
        go.Scatter(
            x=intervalo_x_completo, y=intervalo_y_completo, mode="lines", name="f(x)"
        )
    )
    fig.add_trace(go.Scatter(x=puntosx, y=puntosy, mode="markers", name=alias_punto))
    fig.update_layout(
        title=f"Función: {funcion} en intervalo [{xinicial}, {xfinal}]",
        xaxis_title="x",
        yaxis_title="f(x)",
    )
    return fig
...
def spline_plot(fxs, puntosx, puntosy):
    try:
        ...
        for i in range(len(fxs)):
            ...
            fig.add_trace(
                go.Scatter(
                    x=intervalo_x_completo,
                    y=intervalo_y_completo,
                    mode="lines",
                    name=f"s(x{i})={fxs[i]}",
                )
            )
        ...
        fig.update_layout(
            title=f"Funciones spline", xaxis_title="x", yaxis_title="f(x)"
        )
        fig.add_trace(
            go.Scatter(x=puntosx, y=puntosy, mode="markers", name="Puntos dados")
        )
        return fig
