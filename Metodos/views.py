        ...
        try:
            ...
            if "errors" in datos and datos["errors"]:
                context["error_message"] = (
                    f'Hubo un error en el metodo Regla Falsa en: {datos["errors"]}'
                )
            ...
            if "results" in datos:
                ...
                grafico_path = os.path.join(
                    settings.BASE_DIR, "Metodos/static/graficoa.svg"
                )
                ...
                plot_html = (
                    f'<img src="/static/graficoa.svg" alt="Gráfico de la función">'
                )
                ...
                context = {
                    "df": df.to_html(),
                    "plot_html": plot_html,
                    "mensaje": f'La solución es: {datos["root"]}',
                    "nombre_metodo": "Regla Falsa",
                }
        except Exception as e:
            context = {
                "error_message": f"Hubo un error en el metodo Regla Falsa en: {str(e)}"
            }
            return render(request, "error.html", context)
    ...
    try:
        if Fun.subs(x, a) * Fun.subs(x, b) > 0:
            output["errors"].append(
                "La funcion no cambia de signo en el intervalo dado"
            )
            return output
...
def puntoFijo(X0, Tol, Niter, fx, gx, tipo_error):
    ...
    try:
        ...
        plt.title(
            f"Funciones: f(x) = {fx} y g(x) = {gx} en intervalo [{X0 - 5}, {X0 + 5}]"
        )
        plt.xlabel("x")
        plt.ylabel("f(x) / g(x)")
        plt.grid(True)
    ...
    if request.method == "POST":
        try:
            ...
            try:
                ...
                raise ValueError(
                    "El número de iteraciones no es un valor entero válido."
                )
            ...
            if datos["results"]:
                context = {
                    "df": df.to_html(),
                    "plot_html": plot_html,
                    "mensaje": f"La solución es: {root_value}",
                    "nombre_metodo": "Punto Fijo",
                }
                return render(request, "one_method.html", context)

        except Exception as e:
            context = {
                "error_message": f"Hubo un error en el método de Punto Fijo: {str(e)}"
            }
            return render(request, "error.html", context)
...
def biseccion(request):
    if request.method == "POST":
        ...
        try:
            ...
            if hay_solucion:
                plt.scatter(
                    [float(solucion)], [0], color="red", zorder=5, label="Raíz hallada"
                )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    "Los valores ingresados no son válidos. Asegúrese de que x0, x1, tol y niter sean números válidos."
                )
            ...
            if f0 == 0:
                ...
                if f2 == 0:
                    ...
                    while itera < niter and tol < error and f1 != 0 and f0 != 0:
                        ...
                        absoluto = abs(x2 - x1)
                        tabla.append(
                            [itera, x0, x1, x2, f0, f1, f2, absoluto, relativo]
                        )
                        x0 = x1
                        f0 = f1
                        x1 = x2
                    ...
                    if error < tol:
                        mensaje += (
                            "La solución aproximada que cumple la tolerancia es: "
                            + str(x2)
                            + " En la iteración "
                            + str(itera)
                            + " = x"
            ...
            ax.scatter(
                [x0, x1, solucion], [f0, f1, fsolucion], color="red", label="Raíces"
            )
    ...
    try:
        ...
        while Error > Tol and f != 0 and derivada != 0 and c < Niter:
            ...
            datos.append(
                [
                    c,
                    "{:^15.7f}".format(float(xi)),
                    "{:^15.7E}".format(float(f)),
                    "{:^15.7E}".format(float(Error)),
                ]
            )
    ...
    if request.method == "POST":
        try:
            ...
            if "results" in datos:
                ...
                plt.title(
                    f"Función: {fx} en intervalo [{xi_copy - 10}, {xs_copy + 10}]"
                )
                plt.xlabel("x")
                plt.ylabel("f(x)")
                plt.legend()
            ...
            context = {
                "error_message": f"Hubo un error en el método de Newton: {str(e)}"
            }
            return render(request, "error.html", context)
...
def raicesMultiples(fx, x0, tol, niter, tipo_error):
    ...
    while (error > tol) and (i < niter):
        if i == 0:
            ...
            if d_ex2 == 0 or d2_ex2 == 0:
                output["errors"].append(
                    "Error: La primera o la segunda derivada son cero."
                )
                return output
            ...
            results.append(
                [
                    i,
                    "{:^15.7E}".format(float(xA)),
                    "{:^15.7E}".format(float(ex_2)),
                    "{:^15.7E}".format(float(error)),
                ]
            )
        i += 1
    ...
    if request.method == "POST":
        try:
            ...
            context = {
                "df": df.to_html(),
                "mensaje": f'La solución es: {datos["root"]}',
                "grafico_path": "/static/graficoa.svg",
                "nombre_metodo": "Raices Multiples",
            }
    ...
    if request.method == "POST":
        ...
        try:
            ...
            for i in range(n - 1):
                if xi[i] >= xi[i + 1]:
                    context = {
                        "error_message": f"Los valores de x deben estar ordenados de menor a mayor"
                    }
                    return render(request, "error.html", context)
            ...
            if metodo_interpolacion == "lagrange":
                try:
                    ...
                    for i in range(n):
                        ...
                        iteraciones.append({"i": i, "L_i": Li_str})
                    ...
                    plt.figure(figsize=(10, 6))
                    plt.plot(
                        intervalo_x_completo,
                        intervalo_y_completo,
                        label="f(x)",
                        color="blue",
                    )
                    plt.axhline(0, color="black", linewidth=1)
                    ...
                    plt.title(
                        f"Función: {polisimple} en intervalo [{a - 10}, {b + 10}]"
                    )
                    plt.xlabel("x")
                    plt.ylabel("f(x)")
                    plt.legend()
                    ...
                    context = {
                        ...
                        "df": df_iteraciones.to_html(
                            classes="table table-striped", index=False
                        ),
                        "mensaje": mensaje,
                        "plot_html": plot_html,
                        "nombre_metodo": "Lagrange",
                    }
                    ...
                    context = {
                        "error_message": f"Hubo un error con lagrange en: {str(e)}"
                    }
                    return render(request, "error.html", context)
                ...
                try:
                    ...
                    diff_table_df_to_html = diff_table_df.to_html(
                        classes="table table-striped", index=False
                    )
                    ...
                    newton_poly_df_to_html = newton_poly_df.to_html(
                        classes="table table-striped", index=False
                    )
                    ...
                    context = {
                        "error_message": f"Hubo un error con Newton Diferencias Divididas en: {str(e)}"
                    }
                    return render(request, "error.html", context)
                ...
                try:
                    ...
                    y_vals = [
                        sum([nvectora[i][0] * (val ** (n - 1 - i)) for i in range(n)])
                        for val in x_vals
                    ]
                    ...
                    plt.plot(
                        x_vals, y_vals, label="Polinomio de Vandermonde", color="blue"
                    )
                    ...
                    context = {
                        "error_message": f"Hubo un error con vandermonde: {str(e)}"
                    }
                    return render(request, "error.html", context)
                ...
                coef_df = pd.DataFrame(
                    coef_list, columns=["Pendiente (m)", "Intersección (b)"]
                )
                coef_df_to_html = coef_df.to_html(
                    classes="table table-striped", index=False
                )

                func_df = pd.DataFrame({"Función": [str(func) for func in px_tabla]})
                func_df_to_html = func_df.to_html(
                    classes="table table-striped", index=False
                )
            ...
            context = {
                "error_message": f"Ocurrió un error en la obtención de los datos, llene todos los campos bien, numeros enteros o decimales"
            }
            return render(request, "error.html", context)
